...
;-----------------------------------------------------------------------------------------
; Constant
#define CompanyDir      "HwangChangho"
#define AppPublisher    "HwangChangho Co., Ltd."
#define AppPublisherURL "http://www.hwangchangho.com/"
#define AppID           "amcap"
;-----------------------------------------------------------------------------------------

;-----------------------------------------------------------------------------------------
; Version
#define AppVerName      "1.00.00"
...
;-----------------------------------------------------------------------------------------
; Server/Client
#define APP_NAME_SERVER         "amcap"
#define APP_NAME_CLIENT         "amcap"
...
[Setup]
    ...
    OutputBaseFilename=.\{#AppName}_setup({#LANG_CURRENT_NAME}_V{#AppVerName}.{#AppBuildVer})
    ;SetupIconFile: 설치 실행파일 아이콘
    SetupIconFile=.\mfc.ico
    ;Password: 설치시 비밀번호
    ;Compression: 압축방식
    Compression=lzma
...
[Files]
    ...
    ;-----------------------------------------------------------------------------------------
    ; Programs
    Source: "..\Release\amcap.exe";         DestDir: {app};    Flags: ignoreversion;
    ;-----------------------------------------------------------------------------------------
...
[Icons]
    ;-----------------------------------------------------------------------------------------
    ; IrisKeyDemo icon
    Name: {group}\amcap;                         Filename: {app}\amcap.exe
    Name: {group}\{cm:UninstallProgram,{#AppName}};    Filename: {uninstallexe}
    Name: {commondesktop}\amcap;                 Filename: {app}\amcap.exe; Tasks: desktopicon
    ;-----------------------------------------------------------------------------------------
...
[Run]
    ...
    ;-----------------------------------------------------------------------------------------
    ; IrisKeyDemo.exe run
    Filename: {app}\amcap.exe;    Description: {cm:LaunchProgram,IrisKeyDemo};    Flags: nowait postinstall skipifsilent skipifdoesntexist
    ;-----------------------------------------------------------------------------------------
...
[Code]
    ...
    begin
        ...
        if IsModuleLoaded( 'amcap.exe' ) then
        begin
            MsgBox( 'amcap.exe is running, please close it and run again setup.', mbError, MB_OK );
            Result := false;
        end
        else Result := true;
        //--------------------------------------------------------------------------------------------------
    ...
    begin
        ...
        if IsModuleLoadedU( 'amcap.exe' ) then
        begin
            MsgBox( 'amcap is running, please close it and run again uninstall.', mbError, MB_OK );
            Result := false;
        end
        else Result := true;
