...
PortStatus PortCheck (LPCTSTR lpszDevice)
{
...
//------------------------------------------------------------------------------
//2015.11.10 added by hwang for tray
#include <Shellapi.h>
#include <Shlwapi.h>
...
//------------------------------------------------------------------------------
//2015.11.09 added by hwang for serial port
BOOL CALLBACK ConfigSerialDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//2015.11.11 added by hwang for thread
TCHAR gszSerialPort[] = TEXT("COM1");
...
int ShowError (LONG lError, LPCTSTR lptszMessage)
{
	...
	wsprintf(tszMessage,TEXT("%s\n(error code %d)"), lptszMessage, lError);

	// Display message-box and return with an error-code
	::MessageBox(0,tszMessage,TEXT("Listener"), MB_ICONSTOP|MB_OK);
	return 1;
}
...
{ 
	...
	lLastError = serial.Open(gszSerialPort,0,0,false);
	if (lLastError != ERROR_SUCCESS)
		return ::ShowError(serial.GetLastError(), TEXT("Unable to open COM-port"));

	// Setup the serial port (9600,8N1, which is the default setting)
	lLastError = serial.Setup(CSerial::EBaud9600,CSerial::EData8,CSerial::EParNone,CSerial::EStop1);
	if (lLastError != ERROR_SUCCESS)
		return ::ShowError(serial.GetLastError(), TEXT("Unable to set COM-port setting"));
	...
	lLastError = serial.SetMask(CSerial::EEventBreak |
		...
		CSerial::EEventRecv);
	if (lLastError != ERROR_SUCCESS)
		return ::ShowError(serial.GetLastError(), TEXT("Unable to set COM-port event mask"));
	...
	if (lLastError != ERROR_SUCCESS)
		return ::ShowError(serial.GetLastError(), TEXT("Unable to set COM-port read timeout."));
	...
	do
	{
		...
		if (lLastError != ERROR_SUCCESS)
			return ::ShowError(serial.GetLastError(), TEXT("Unable to wait for a COM-port event."));
		...
		if (eEvent & CSerial::EEventRecv)
		{
			...
			do
			{
				...
				if (lLastError != ERROR_SUCCESS)
					return ::ShowError(serial.GetLastError(), TEXT("Unable to read from COM-port."));
				...
				if (dwBytesRead > 0)
				{
					...
					if ( strcmp(szBuffer, "hide") == 0 )
						ShowWindow(ghwndApp, SW_HIDE);
					else if ( strcmp(szBuffer, "show") == 0 )
...
BOOL AppInit(HINSTANCE hInst, HINSTANCE hPrev, int sw)
{
    ...
    if(!hPrev)
    {
        ...
        */
        ...
        //cls.hIcon          = NULL;
		cls.hIcon          = LoadIcon(hInst, MAKEINTRESOURCE(IDR_MAINFRAME));
    ...
    *szAudioDisplayName = *szVideoDisplayName = 0; // null terminate
	...
	// load the icon
	niData.hIcon = (HICON)LoadImage(hInst, MAKEINTRESOURCE(IDR_MAINFRAME), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON),GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR);

	// the window to send messages to and the message to send
	//		note:	the message value should be in the
	//				range of WM_APP through 0xBFFF
	niData.hWnd = ghwndApp;
	niData.uCallbackMessage = SWM_TRAYMSG;

	// tooltip message
	lstrcpyn(niData.szTip, TEXT("Time flies like an arrow but\n   fruit flies like a banana!"), sizeof(niData.szTip)/sizeof(TCHAR));
...
LONG WINAPI  AppWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
        ...
        case WM_CREATE:
			DialogBox(ghInstApp, MAKEINTRESOURCE(IDD_CONFIG_SERIAL), hwnd, (DLGPROC)ConfigSerialDlgProc);
            break;
        ...
        case WM_INITMENU:
            ...
            EnableMenuItem((HMENU)wParam, MENU_START_CAP, (!gcap.fCapturing) ? MF_ENABLED : MF_GRAYED);

            // we can stop capture if it's currently capturing
            EnableMenuItem((HMENU)wParam, MENU_STOP_CAP, (gcap.fCapturing) ? MF_ENABLED : MF_GRAYED);

            // We can bring up a dialog if the graph is stopped
            EnableMenuItem((HMENU)wParam, MENU_DIALOG0, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG1, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG2, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG3, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG4, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG5, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG6, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG7, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG8, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOG9, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOGA, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOGB, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOGC, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOGD, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOGE, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_DIALOGF, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);

            // toggles capturing audio or not - can't be capturing right now
            // and we must have an audio capture device created
            EnableMenuItem((HMENU)wParam, MENU_CAP_AUDIO, (!gcap.fCapturing && gcap.pACap) ? MF_ENABLED : MF_GRAYED);
            // are we capturing audio?
            CheckMenuItem((HMENU)wParam, MENU_CAP_AUDIO, (gcap.fCapAudio) ? MF_CHECKED : MF_UNCHECKED);
            // are we doing closed captioning?
            CheckMenuItem((HMENU)wParam, MENU_CAP_CC, (gcap.fCapCC) ? MF_CHECKED : MF_UNCHECKED);
            EnableMenuItem((HMENU)wParam, MENU_CAP_CC, (gcap.fCCAvail) ? MF_ENABLED : MF_GRAYED);
            // change audio formats when not capturing
            EnableMenuItem((HMENU)wParam, MENU_AUDIOFORMAT, (gcap.fCapAudio && !gcap.fCapturing) ? MF_ENABLED : MF_GRAYED);
            // change frame rate when not capturing, and only if the video
            // filter captures a VIDEOINFO type format
            EnableMenuItem((HMENU)wParam, MENU_FRAMERATE, (!gcap.fCapturing && gcap.fCapAudioIsRelevant) ? MF_ENABLED : MF_GRAYED);
            // change time limit when not capturing
            EnableMenuItem((HMENU)wParam, MENU_TIMELIMIT, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            // change capture file name when not capturing
            EnableMenuItem((HMENU)wParam, MENU_SET_CAP_FILE, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            // pre-allocate capture file when not capturing
            EnableMenuItem((HMENU)wParam, MENU_ALLOC_CAP_FILE, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            // can save capture file when not capturing
            EnableMenuItem((HMENU)wParam, MENU_SAVE_CAP_FILE, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            // do we want preview?
            CheckMenuItem((HMENU)wParam, MENU_PREVIEW, (gcap.fWantPreview) ? MF_CHECKED : MF_UNCHECKED);
            // can toggle preview if not capturing
            EnableMenuItem((HMENU)wParam, MENU_PREVIEW, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);

            // MPEG2 enabled ?
            CheckMenuItem((HMENU)wParam, MENU_MPEG2, (gcap.fMPEG2) ? MF_CHECKED : MF_UNCHECKED);

            // can toggle MPEG2 if not capturing
            EnableMenuItem((HMENU)wParam, MENU_MPEG2, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);


            // which is the master stream? Not applicable unless we're also
            // capturing audio
            EnableMenuItem((HMENU)wParam, MENU_NOMASTER, (!gcap.fCapturing && gcap.fCapAudio) ? MF_ENABLED : MF_GRAYED);
            CheckMenuItem((HMENU)wParam, MENU_NOMASTER, (gcap.iMasterStream == -1) ? MF_CHECKED : MF_UNCHECKED);
            EnableMenuItem((HMENU)wParam, MENU_AUDIOMASTER, (!gcap.fCapturing && gcap.fCapAudio) ? MF_ENABLED : MF_GRAYED);
            CheckMenuItem((HMENU)wParam, MENU_AUDIOMASTER, (gcap.iMasterStream == 1) ? MF_CHECKED : MF_UNCHECKED);
            EnableMenuItem((HMENU)wParam, MENU_VIDEOMASTER, (!gcap.fCapturing && gcap.fCapAudio) ? MF_ENABLED : MF_GRAYED);
            CheckMenuItem((HMENU)wParam, MENU_VIDEOMASTER, (gcap.iMasterStream == 0) ? MF_CHECKED : MF_UNCHECKED);

            // can't select a new capture device when capturing
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE0, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE1, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE2, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE3, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE4, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE5, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE6, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE7, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE8, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_VDEVICE9, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE0, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE1, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE2, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE3, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE4, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE5, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE6, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE7, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE8, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
            EnableMenuItem((HMENU)wParam, MENU_ADEVICE9, !gcap.fCapturing ? MF_ENABLED : MF_GRAYED);
        ...
        //
        case WM_DESTROY:
			//2015.11.09 deleted by hwang for serial port
			/*
            ...
            PostQuitMessage(0);
			*/
			ShowWindow(hwnd, SW_HIDE);	//2015.11.09 added by hwang for serial port
            break;


        case WM_CLOSE:
            //OnClose();				//2015.11.09 deleted by hwang for serial port
			ShowWindow(hwnd, SW_HIDE);	//2015.11.09 added by hwang for serial port
            break;
...
LONG PASCAL AppCommand(HWND hwnd, unsigned msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(id)
    {
        ...
        //
        case MENU_EXIT:
            //PostMessage(hwnd,WM_CLOSE,0,0L);	//2015.11.09 deleted by hwang for serial port
			ShowWindow(hwnd, SW_HIDE);				//2015.11.09 added by hwang for serial port
            break;
...
BOOL CALLBACK ConfigSerialDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch(msg)
    {
        case WM_COMMAND:
			{
				//EndDialog(hwnd, TRUE);
				HWND hcom1 = GetDlgItem(hwnd, IDC_COM1);
				HWND hcom2 = GetDlgItem(hwnd, IDC_COM2);
				HWND hcom3 = GetDlgItem(hwnd, IDC_COM3);
				HWND hcom4 = GetDlgItem(hwnd, IDC_COM4);
				HWND hcom5 = GetDlgItem(hwnd, IDC_COM5);
				HWND hcom6 = GetDlgItem(hwnd, IDC_COM6);
				HWND hcom7 = GetDlgItem(hwnd, IDC_COM7);
				HWND hcom8 = GetDlgItem(hwnd, IDC_COM8);
				HWND hcom9 = GetDlgItem(hwnd, IDC_COM9);

				if (IsDlgButtonChecked(hwnd, IDC_COM1) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM1"), TEXT("COM1"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM1"));
				}
				else if (IsDlgButtonChecked(hwnd, IDC_COM2) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM2"), TEXT("COM2"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM2"));
				}
				else if (IsDlgButtonChecked(hwnd, IDC_COM3) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM3"), TEXT("COM3"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM3"));
				}
				else if (IsDlgButtonChecked(hwnd, IDC_COM4) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM4"), TEXT("COM4"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM4"));
				}
				else if (IsDlgButtonChecked(hwnd, IDC_COM5) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM5"), TEXT("COM5"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM5"));
				}
				else if (IsDlgButtonChecked(hwnd, IDC_COM6) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM6"), TEXT("COM6"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM6"));
				}
				else if (IsDlgButtonChecked(hwnd, IDC_COM7) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM7"), TEXT("COM7"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM7"));
				}
				else if (IsDlgButtonChecked(hwnd, IDC_COM8) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM8"), TEXT("COM8"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM8"));
				}
				else if (IsDlgButtonChecked(hwnd, IDC_COM9) == BST_CHECKED)
				{
					MessageBox(hwnd, TEXT("COM9"), TEXT("COM9"), MB_OK);
					_tcscpy(gszSerialPort, TEXT("COM9"));
				}
				else
				{
					MessageBox(hwnd, TEXT("You should choice port!"), TEXT("COM#"), MB_OK);
					return TRUE;
				}
        ...
        case WM_INITDIALOG:
			{
				...
				EnableWindow(hcom1, PortCheck(TEXT("COM1")) == CSerial::EPortAvailable);
				EnableWindow(hcom2, PortCheck(TEXT("COM2")) == CSerial::EPortAvailable);
				EnableWindow(hcom3, PortCheck(TEXT("COM3")) == CSerial::EPortAvailable);
				EnableWindow(hcom4, PortCheck(TEXT("COM4")) == CSerial::EPortAvailable);
				EnableWindow(hcom5, PortCheck(TEXT("COM5")) == CSerial::EPortAvailable);
				EnableWindow(hcom6, PortCheck(TEXT("COM6")) == CSerial::EPortAvailable);
				EnableWindow(hcom7, PortCheck(TEXT("COM7")) == CSerial::EPortAvailable);
				EnableWindow(hcom8, PortCheck(TEXT("COM8")) == CSerial::EPortAvailable);
				EnableWindow(hcom9, PortCheck(TEXT("COM9")) == CSerial::EPortAvailable);

				CheckRadioButton(hwnd, IDC_COM1, IDC_COM9, IDC_COM1);
			}
			return TRUE;
    }
    return FALSE;
